...
| `components/` | 20 componentes, um `.tsx` + `.module.css` por componente, agrupados em `brand/`, `actions/`, `display/`, `cards/`, `forms/`, `navigation/`. API pública em `components/index.ts` (`@ds/components`). Uso de cada um: [`components/README.md`](components/README.md). |
